			...
			for (int i = 0; i <= goodlist.size() - 1; i++) {
				for (int j = 0; j <= listC.size() - 1; j++) {
					if (goodlist.get(i).getfood_cd().toString().equals(listC.get(j))) {
						/* System.out.print("list의 get "+goodlist.get(i).getfood_cd().toString());
						System.out.print("list의 get(j)"+listC.get(j)); */	
						flag2 = false;
					}
				}				
				if(flag2){
					System.out.println("cd는뭐니"+goodlist.get(i).getfood_cd());
					goodlist.remove(i);
				}
				flag2 = true;
			}
