...
<body>
	...
	<h2>
		...
		<%
			ArrayList<String> goodimg = new ArrayList<>();
		%>
		<%
			ArrayList<String> goodname = new ArrayList<>();
		%>
		<%
			ArrayList<String> badimg = new ArrayList<>();
		%>
		<%
		...
		<%
			String[] sickname = { "질병", "치주질환", "기관지염/비염", "고혈압", "척추질환", "식도/위/식이지장염", "장염/결장염", "피부염", "당뇨병", "백내장",
					"무릎관절증", "어지럼증", "어깨병변" };
		%>
		...
		%>
		<%
			goodimg.add(goodlist.get(j).getfood_img());
					goodname.add(goodlist.get(j).getfood_name());
					System.out.println(goodimg.toString());
					System.out.println(goodname.toString());	%>


		<img src="<%=goodlist.get(j).getfood_img()%>"> <br>
		...
		%>
		<img src="<%=badlist.get(j).getfood_img()%>"> <br>
