...
<body>
	...
	%>
	<button id ="food<%=i%>" name="food" value=<%=Integer.parseInt(foodname.get(i).getfood_cd())%>><%=foodname.get(i).getfood_name()%></button>
	<%
		}
	%>
	...
	/*20200601 희정 자바코드 자바스크립트로 변환 시작  */
		...
		var array = new Array();
		array=<%=food_cd2 %>;
		var pushpushbaby=function (food){
			if(food === undefined){
				food = 1;
			}
			return food;
