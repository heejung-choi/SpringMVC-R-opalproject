...
write.csv(final,file="opal1_c.csv")
...
library(wordcloud2)
(words <- read.csv("C:/Eunha/opal1_c.csv",stringsAsFactors = F))


#원 기울여
opalstomach<-wordcloud2(words,fontFamily = "Yang Rounded",size=1.0,col="random-light",backgroundColor = "black",rotateRatio = 1 ,minRotation = -pi/6, maxRotation = -pi/6)
#삼각형
opalstomach1<-wordcloud2(words, shape = 'triangle-forward',fontFamily = "Yang Rounded",size=1.0,col="random-light",backgroundColor = "black",rotateRatio = 0 ,minSize = 0.5)

library("htmlwidgets")   # 결과를 html로 만들어 주는 도우미 패키지
#원 기울여
saveWidget(opalstomach,"C:/Eunha/opalstomach.html",selfcontained = F)
#삼각형
saveWidget(opalstomach1,"C:/Eunha/opalstomach_T.html",selfcontained = F)
