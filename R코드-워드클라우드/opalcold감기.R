...
query <- URLencode(iconv("감기 건강음식","euc-kr","UTF-8")) # iconv 함수를 이용해 봄이라는 문자열을  euc-kr을 이용해서 utf-8이 쿼리문자열이 만들어 지게끔// 쿼리문자열 구성하기 위해, URL 인코드 하는 ... 쿼리 문자열 , 영문자 , 일부 특수 문자 빼고는 %와 함께 16진수로 간다.
...
text1<-text #검색어:감기에 좋은 음식
text2<-text #검색어: 기관지염에 좋은 음식
finish2<-append(text1,text2)
finish2
text3<-text #검색어: 호흡기 질환에 좋은 음식
finish2<-append(finish2,text3)
text4<-text #검색어: 기관지염 건강식품
finish2<-append(finish2,text4)
text5<-text #호흡기질환 건강식품
...
write.csv(final,"C:/Eunha/opal2.csv")
...
library(wordcloud2)
(words <- read.csv("C:/Eunha/opal2.csv",stringsAsFactors = F))

#원 기울여
opalcold<-wordcloud2(words,fontFamily = "Yang Rounded",size=1.0,col="random-light",backgroundColor = "black",rotateRatio = 1 ,minRotation = -pi/6, maxRotation = -pi/6)
#삼각형
