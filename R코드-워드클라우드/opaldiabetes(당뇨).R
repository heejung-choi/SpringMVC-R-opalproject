...
query <- URLencode(iconv("당뇨병 건강음식","euc-kr","UTF-8")) # iconv 함수를 이용해 봄이라는 문자열을  euc-kr을 이용해서 utf-8이 쿼리문자열이 만들어 지게끔// 쿼리문자열 구성하기 위해, URL 인코드 하는 ... 쿼리 문자열 , 영문자 , 일부 특수 문자 빼고는 %와 함께 16진수로 간다.
...
text1<-text #검색어:당뇨에 좋은 음식
text2<-text #검색어: 당뇨병에 좋은 음식
finish2<-append(text1,text2)
finish2
text3<-text #검색어:  제2형 당뇨병에 좋은 음식
finish2<-append(finish2,text3)
text4<-text #검색어: 제1형 당뇨병에 좋은 음식에 좋은 음식
...
View(final)
write.csv(final,"C:/Eunha/opal4.csv")
...
library(wordcloud2)
(words <- read.csv("C:/Eunha/opal4.csv",stringsAsFactors = F))

#원 기울여 
opaldiabetes<-wordcloud2(words,fontFamily = "Yang Rounded",size=1.0,col="random-light",backgroundColor = "black",rotateRatio = 1 ,minRotation = -pi/6, maxRotation = -pi/6)
#삼각형
opaldiabetes1<-wordcloud2(words, shape = 'triangle-forward',fontFamily = "Yang Rounded",size=1.0,col="random-light",backgroundColor = "black",rotateRatio = 0 ,minSize = 0.5)


library("htmlwidgets")   # 결과를 html로 만들어 주는 도우미 패키지

#원 기울여
saveWidget(opaldiabetes,"C:/Eunha/opaldiabetes.html",selfcontained = F)
#삼각형
saveWidget(opaldiabetes1,"C:/Eunha/opaldiabetes_T.html",selfcontained = F)
